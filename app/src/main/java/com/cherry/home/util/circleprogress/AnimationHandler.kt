...
class AnimationHandler internal constructor(circleView: CircleProgressView) : Handler(circleView.getContext().getMainLooper()) {

    private val mCircleViewWeakReference: WeakReference<CircleProgressView>
    // Spin bar length in degree at start of animation
    private var mSpinningBarLengthStart: Float = 0.toFloat()
    ...
    private var mLengthChangeAnimationDuration: Double = 0.0
    private var mFrameStartTime: Long = 0

    init {
        mCircleViewWeakReference = WeakReference<CircleProgressView>(circleView)
    }
        ...
        when (circleView.mAnimationState) {
            ...
            AnimationState.SPINNING -> when (msgType) {
                ...
                AnimationMsg.TICK -> {
                    ...
                    val length_delta = circleView.mSpinningBarLengthCurrent - circleView.mSpinningBarLengthOrig
                    var t = ((System.currentTimeMillis() - mLengthChangeAnimationStartTime) / mLengthChangeAnimationDuration).toFloat()
                    t = if (t > 1.0f) 1.0f else t
                    val interpolatedRatio = mLengthChangeInterpolator.getInterpolation(t)
                    ...
                    sendEmptyMessageDelayed(AnimationMsg.TICK.ordinal, circleView.delayMillis - (SystemClock.uptimeMillis() - mFrameStartTime))
    ...
    private fun enterSetValueAnimated(msg: Message, circleView: CircleProgressView) {
    ...
    private fun enterEndSpinningStartAnimating(circleView: CircleProgressView, msg: Message) {
    ...
    private fun initReduceAnimation(circleView: CircleProgressView) {
        ...
        mLengthChangeAnimationDuration = (stepsTillFinish * circleView.delayMillis * 2f).toDouble()
    ...
    private fun enterSpinning(circleView: CircleProgressView) {
        ...
        circleView.mSpinningBarLengthCurrent = 360f / circleView.maxValue * circleView.mCurrentValue
        circleView.mCurrentSpinnerDegreeValue = 360f / circleView.maxValue * circleView.mCurrentValue
        mLengthChangeAnimationStartTime = System.currentTimeMillis()
        mSpinningBarLengthStart = circleView.mSpinningBarLengthCurrent


        //calc animation time
        val stepsTillFinish = circleView.mSpinningBarLengthOrig / circleView.spinSpeed
        mLengthChangeAnimationDuration = ((stepsTillFinish * circleView.delayMillis) * 2f).toDouble()
    ...
    private fun calcNextAnimationValue(circleView: CircleProgressView): Boolean {
        var t = ((System.currentTimeMillis() - mAnimationStartTime) / circleView.mAnimationDuration).toFloat()
        t = if (t > 1.0f) 1.0f else t
